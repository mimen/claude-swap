...
def test_unswitchable_oauth_account_disables_all_activation_paths() -> None:
    entry = UsageEntry(last_good={"five_hour": {"pct": 10.0}})
    snapshot = AccountsSnapshot(
        active_number="1",
        accounts=(
            _account("1", entry, active=True),
            _account("2", entry, switchable=False),
        ),
        taken_at=_NOW,
    )

    active, broken = viewmodel.popover_view_model(snapshot, _NOW).accounts

    assert active.switchable
    assert not broken.switchable
    assert not broken.session_available
...
def test_popover_model_retains_last_good_below_sentinel_and_rolls_weekly() -> None:
    ...
    assert stale.sentinel_note == viewmodel.SENTINEL_NOTES[USAGE_TOKEN_EXPIRED]
