...
def _deliver_notification(title: str, message: str, *, center=None) -> None:
    """Deliver a native Notification Center message through the Python host."""
    notification = Foundation.NSUserNotification.alloc().init()
    notification.setTitle_(title)
    notification.setInformativeText_(message)
    target = center or Foundation.NSUserNotificationCenter.defaultUserNotificationCenter()
    target.deliverNotification_(notification)


def _deliver_notification_if_hidden(
    popover_shown: bool,
    title: str,
    message: str,
    *,
    deliver: Callable[[str, str], None] = _deliver_notification,
) -> None:
    """Deliver background feedback only when the popover cannot show it."""
    if not popover_shown:
...
def _create_native_menubar(switcher: ClaudeAccountSwitcher):
    ...
    class PopoverContentController(AppKit.NSViewController):
        ...
        def _account_group(
            ...
            make_active = self._button(
                "Make active",
                "makeActive:",
                self._local(layout.make_active, layout.frame),
                slot=account.number,
                accessibility_label=(
                    f"Make active, Account-{account.number}, {account.email}"
                ),
            )
            make_active.setEnabled_(account.switchable)
            group.addSubview_(make_active)
            if account.session_available:
                session_button = self._button(
                    ...
                    slot=account.number,
                    accessibility_label=(
                        f"Session, Account-{account.number}, {account.email}"
                    ),
                )
            else:
                session_title = (
                ...
                session_button = self._button(
                    ...
                    self._local(layout.session, layout.frame),
                    accessibility_label=(
                        f"{session_title}, Account-{account.number}, {account.email}"
                    ),
                )
        ...
        def _usage_row(self, row: UsageRowViewModel, frame: Rect, parent) -> None:
            ...
            accessibility = f"{row.label}, {row.state_label}, {suffix}"
            if row.ahead_of_pace:
                accessibility += ", ahead of pace"
            field.setAccessibilityLabel_(accessibility)
            ...
            for account, frame in zip(model.accounts, layout.items, strict=False):
                ...
                document.addSubview_(
                    self._button(
                        ...
                        slot=account.number,
                        accessibility_label=(
                            f"{state}, Account-{account.number}, {account.email}"
                        ),
                    )
                )
                document.addSubview_(
                    self._button(
                        ...
                        slot=account.number,
                        accessibility_label=(
                            f"Remove, Account-{account.number}, {account.email}"
                        ),
                    )
                )
    ...
    class NativeMenuBarHost(Foundation.NSObject):
        ...
        def initWithSwitcher_(self, initial_switcher):
            ...
            self.controller.bind_ui(self._render, self._message, self._notification)
        ...
        @objc.python_method
        def _notification(self, title: str, message: str) -> None:
            _deliver_notification_if_hidden(
                bool(self.popover.isShown()),
                title,
                message,
                deliver=_deliver_notification,
            )
        ...
        @objc.python_method
        def _account(self, slot: str) -> PopoverAccountViewModel | None:
            return next(
                (
                    account
                    for account in self.controller.view_model.accounts
                    if account.number == slot
                ),
                None,
            )
        ...
        def makeActive_(self, sender) -> None:
            ...
            if account is None or not account.switchable:
                self._message(
                    ...
                    f"Account {slot} is unavailable because its credentials or configuration are incomplete.",
                )
                return
        ...
        def isolatedSession_(self, sender) -> None:
            slot = str(sender.representedObject())
            account = self._account(slot)
            if account is None or not account.session_available:
                self._message(
